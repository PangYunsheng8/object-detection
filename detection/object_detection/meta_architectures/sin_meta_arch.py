...
class SINMetaArch(model.DetectionModel):
  ...
  def __init__(self,
    ...
    """FasterRCNNMetaArch based SIN Constructor.
    ...
    Args:
      ...
      second_stage_box_predictor: Mask R-CNN box predictor to use for
        the second stage.
    ...
    """
    super(SINMetaArch, self).__init__(num_classes=num_classes)
    ...
    self._second_stage_box_predictor = second_stage_box_predictor
  ...
  def _predict_second_stage(self, rpn_box_encodings,
    ...
    rpn_feature_shape = shape_utils.combined_static_and_dynamic_shape(rpn_features_to_crop)
    batch_size = rpn_feature_shape[0]
    ...
    object_feature_vectors = tf.reduce_mean(flattened_object_features, [1, 2],
                                                     keep_dims=True,
                                                     name='AvgPool_obj') 
    object_feature_vectors = slim.flatten(object_feature_vectors)
    feature_length = shape_utils.combined_static_and_dynamic_shape(object_feature_vectors)[-1]
    object_feature_vectors = tf.reshape(object_feature_vectors, 
                                [batch_size, self.max_num_proposals, feature_length])
    
    context_features = (
        self._feature_extractor.extract_box_classifier_features(
            rpn_features_to_crop,
            scope=self.second_stage_feature_extractor_scope))
    context_feature_vectors = tf.reduce_mean(context_features, [1, 2],
                                                     keep_dims=True,
                                                     name='AvgPool_ctx') 
    context_feature_vectors = slim.flatten(context_feature_vectors)
    context_feature_vectors = tf.tile(tf.expand_dims(context_feature_vectors, 1),
                                [1, self.max_num_proposals, 1])
    flattened_context_vectors = self._flatten_first_two_dimensions(context_feature_vectors)

    spatial_vectors = self._compute_spatial_vextors(proposal_boxes_normalized)
    with slim.arg_scope(self._fc_hyperparams):
        spatial_e = slim.fully_connected(spatial_vectors, 1, scope="PE")

    cell_context = rnn.GRUCell(feature_length, name="Context_GRU")
    cell_edge    = rnn.GRUCell(feature_length, name="Edge_GRU")
    hidden_state = object_feature_vectors
    n_steps = 3
    
    for i in range(n_steps):

        hidden_state_i = tf.tile(tf.expand_dims(hidden_state, 2),
                                    [1, 1, self.max_num_proposals, 1])
        hidden_state_j = tf.transpose(hidden_state_i, perm=[0,2,1,3])
        hidden_state_matrix = tf.concat([hidden_state_i, hidden_state_j],
                                       axis=-1, name="hidden_state_matrix")
        with slim.arg_scope(self._fc_hyperparams):
            edge_e = slim.fully_connected(hidden_state_matrix, 
                            1,
                            activation_fn=tf.nn.tanh,
                            reuse=tf.AUTO_REUSE,
                            scope="Edge_E")
        e_matrix = spatial_e * edge_e
        mixed_edges = tf.reduce_max(
                        edge_e*hidden_state_j, [2],
                        keep_dims=False, name="integrated_edge_vectors")
        flattened_hidden_state = self._flatten_first_two_dimensions(hidden_state)
        flattened_mixed_edges  = self._flatten_first_two_dimensions(mixed_edges)

        with tf.variable_scope("GRU_CELLS", 
                  values=[flattened_hidden_state, flattened_context_vectors, flattened_mixed_edges],
                  reuse=tf.AUTO_REUSE):
 
            output_c, hidden_state_c = cell_context(inputs=flattened_context_vectors,
                                                    state=flattened_hidden_state)
            output_e, hidden_state_e = cell_edge(inputs=flattened_mixed_edges,
                                                    state=flattened_hidden_state)

        hidden_state = (hidden_state_c + hidden_state_e) / 2.0
        hidden_state = tf.reshape(hidden_state, [batch_size, self.max_num_proposals, feature_length])

    flattened_hidden_state = self._flatten_first_two_dimensions(hidden_state)                                              
    box_predictions = self._second_stage_box_predictor.predict(
    ...
    prediction_dict = {
        ...
        'box_classifier_features': flattened_object_features,
  ...
  def _compute_spatial_vextors(self, proposal_boxes_normalized):
    """Compute spatial vectors.
    """
    proposal_boxes_normalized = tf.tile(
                             tf.expand_dims(proposal_boxes_normalized, 2),
                            [1, 1, self.max_num_proposals, 1])
    ymin_i, xmin_i, ymax_i, xmax_i = tf.split(proposal_boxes_normalized, 
                                              num_or_size_splits=4, axis=3)
    ymin_j, xmin_j, ymax_j, xmax_j = tf.split(
                            tf.transpose(proposal_boxes_normalized, perm=[0, 2, 1, 3]),
                            num_or_size_splits=4, axis=3)
    x_i = (xmin_i + xmax_i) / 2
    y_i = (ymin_i + ymax_i) / 2
    w_i = tf.maximum(0.0001, xmax_i-xmin_i)
    h_i = tf.maximum(0.0001, ymax_i-ymin_i)
    s_i = w_i * h_i

    x_j = (xmin_j + xmax_j) / 2
    y_j = (ymin_j + ymax_j) / 2
    w_j = tf.maximum(0.0001, xmax_j-xmin_j)
    h_j = tf.maximum(0.0001, ymax_j-ymin_j)
    s_j = w_j * h_j
    spatial_vectors = [w_i, h_i, s_i, w_j, h_j, s_j, (x_i-x_j)/w_j, (y_i-y_j)/h_j, 
                       tf.pow((x_i-x_j)/w_j, 2), tf.pow((y_i-y_j)/h_j, 2),
                    tf.log(w_i/w_j), tf.log(h_i/h_j)]
    spatial_vectors = tf.concat(spatial_vectors, axis=-1)
